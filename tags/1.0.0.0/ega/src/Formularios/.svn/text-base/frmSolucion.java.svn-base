...
//    public boolean _blnExecuting = false;
...
//        this.btnVerInformeInicio.setText(VER_INFORME);
    ...
    private void btnProbarConfiguracionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProbarConfiguracionActionPerformed
         this.createFinalSolution();                
            if (this.solutionToUse.CompletedMinimumFieldsTo(Actions.Execute)) {
                 
                ArrayList<Method> listaMetodos = solutionToUse.getArrMethods();
    ...
    private void executionCompleted(){
        ...
        this.solutionToUse.ExecutionCompleted();
        if (this.finished)
    ...
    private void execute(List results, ArrayList<Integer> valores[]){
        if (!canceled){
            ...
            this.solutionToUse.setExecutionResults(Result.ToStore(this.txtHoraInicio.getText(), (Integer) Integer.parseInt(this.txtCiclos.getText()), (Integer) Integer.parseInt(this.txtPoblacionActual.getText()), (Integer) Integer.parseInt(this.txtMutaciones.getText()), (Integer) Integer.parseInt(this.txtAptitudMaxima.getText()), (Integer) Integer.parseInt(this.txtAptitudMinima.getText()), results, iMinima, iMedia, iMaxima));
            String FileName = functions.GetRandomNameForResults(this.solutionToUse.getStrName()) + paths.strSolucionesExtension;
            ...
            this.solutionToUse.SaveResult(ResultPath);
            if (this.solutionToUse.result.OperationSucceed()) {
                this.modified = true;
                this.solutionToUse.getResultEGAFiles().add(new EGAFile(ResultPath, ResultPath));

                this.putDataInListView(this.solutionToUse.getResultEGAFiles(), lstArchivos);
    ...
    private void btnComenzarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnComenzarActionPerformed
          ...
          if (this.btnComenzar.getText().equals(COMENZAR) ) {
            ...
            if (this.solutionToUse.CompletedMinimumFieldsTo(Actions.Execute)) {
                ...
                this.procesoActual.usa(this.solutionToUse);
...
//                this.resetProcessTextBox(this.solutionToUse, today);
...
//        }
    ...
    private void btnEliminarArchivoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarArchivoActionPerformed
        ...
        if (this.fh.FileFound(selectedResource.file.GetPath()).equals(file_handler._fileFound)) {
            ...
            if (this.selectedResource.file.result.OperationSucceed()) {
                ...
                this.solutionToUse.getResultEGAFiles().remove(this.selectedResource.file);
    ...
    private void exportSolution() {
        ...
        if (this.solutionToUse.CompletedMinimumFieldsTo(Actions.Export)) {
            
            String path = this.getSelectedPath(paths.strSoluciones, true);
            if (path.length() > 0){
                String NewFolder = path + "\\" + this.solutionToUse.getStrName();
                ...
                path = NewFolder + "\\" + this.solutionToUse.getStrFile();


                this.exportMethods(this.solutionToUse.getArrMethods(), NewFolder);
                if (this.fh.result.OperationSucceed()) {
                    this.fh.SaveXML(this.solutionToUse, path);
    ...
    private void publishSolution() {
        ...
        if (this.solutionToUse.CompletedMinimumFieldsTo(Actions.Publish)) {
            
            String NewFolder = this.solutionToUse.getStrServerFolder() + "\\" + this.solutionToUse.getStrName();
            ...
            Solution solutionToPublish = (Solution)this.solutionToUse.clone();
    ...
    private boolean saveSolution() {
        this.createFinalSolution();
        if (this.saveTheSolution(this.solutionToUse)){
            this.modified = false;
            return true;
        }else{
            return false;
    ...
    private boolean saveTheSolution(Solution solution) {
        if (solution.CompletedMinimumFieldsTo(Actions.Save)) {
            String strPath = solution.getStrFolder() + "\\" + solution.getStrFile();
            this.save(strPath, solution);
            if (!this.fh.result.OperationSucceed()) {
                JOptionPane.showMessageDialog(this, Messages.strSolutionNotSaved, "EGA", 1);
                return false;
            }

            this.storeSolutionInDatabase(solution);
            if (this.result.OperationSucceed()) {
            ...
            JOptionPane.showMessageDialog(this, Messages.strMinimumFieldsNotCompletedToSave, "EGA", 1);
            return false;
        }
    }

    private void storeSolutionInDatabase(Solution solution) {
        this.ResetOperationStatus();

        ArrayList<Solution> SolutionsInDB = Solution.GetSolutionsInDB();
        Solution.UpdateStoredSolutions(solution, SolutionsInDB);
...
//              modelo.addRow(ROW_VACIA);
    ...
    private void createFinalSolution() {
        ...
        this.solutionToUse.setIntSize(Integer.parseInt(txtPoblacionInicial.getText().trim()));
        this.solutionToUse.setIntIncreasingStyle(this.cmbCrecimientoPoblacion.getSelectedIndex());
        this.solutionToUse.setStrDescription(this.txtDescripcionSolucion.getText().trim());
        this.solutionToUse.setStrObservation(this.txtObservacionSolucion.getText().trim());
        this.solutionToUse.setStrAuthor(this.txtAutor.getText().trim());
        this.solutionToUse.setStrName(this.txtNombreObligatorio.getText().trim());
        this.solutionToUse.setStrCreated(this.txtFechaCreacion.getText().trim());
        this.solutionToUse.setStrModified(this.txtFechaModificacion.getText().trim());
        this.solutionToUse.setStrExecuted(this.txtFechaEjecucion.getText().trim());
        this.solutionToUse.setStrPublished(this.txtFechaPublicacion.getText().trim());
        this.solutionToUse.setStrCompleted(this.txtFechaCompletado.getText().trim());
        this.solutionToUse.setStrFile(this.txtArchivoSolucion.getText().trim() + paths.strSolucionesExtension);
        this.solutionToUse.setStrFolder(this.txtUbicacionArchivoSolucion.getText().trim());
        this.solutionToUse.setBlnReadOnly(chkSoloLectura.isSelected());
        this.solutionToUse.setStrServerFolder(this.txtCarpetaPublica.getText().trim());

        this.solutionToUse.setResultEGAFiles(this.getResourcesInList(lstArchivos));
    }
